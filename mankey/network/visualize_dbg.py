...
def visualize_entry_nostage(
    ...
    # The processed input
    stacked_rgb = dataset[entry_idx][parameter.rgb_image_key]
    normalized_xy = dataset[entry_idx][parameter.keypoint_xy_key]

    stacked_rgb = torch.from_numpy(stacked_rgb)
    ...
    # Combine them
    keypointxy_pred = np.zeros((3, dataset.num_keypoints), dtype=np.int_)
    keypointxy_pred[0, :] = coord_x[0, :, 0].astype(np.int_)
    keypointxy_pred[1, :] = coord_y[0, :, 0].astype(np.int_)

    # Get the image
    from mankey.utils.imgproc import draw_image_keypoint, draw_visible_heatmap
    keypoint_rgb_cv = draw_image_keypoint(processed_entry.cropped_rgb, keypointxy_pred, processed_entry.keypoint_validity)
    rgb_save_path = os.path.join(save_dir, 'image_%d_rgb.png' % entry_idx)
    cv2.imwrite(rgb_save_path, keypoint_rgb_cv)

    # The pixel error
    pixel_error = np.sum(np.sqrt((processed_entry.keypoint_xy[0:2, :] - keypointxy_pred[0:2, :])**2), axis=0)
    max_pixel_error = np.max(pixel_error)
    print('The max pixel error (pixel in 256x256 image) is ', max_pixel_error)

    cv2.imshow('Prediction', keypoint_rgb_cv)
