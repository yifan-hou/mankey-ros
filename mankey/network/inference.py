...
def proc_input_img_internal(
    ...
    stacked_rgb = np.zeros(shape=(channels, height, width), dtype=np.float32)
    stacked_rgb[0:3, :, :] = normalized_rgb
    # stacked_rgb[3, :, :] = normalized_depth

    # OK
    imgproc_out = ImageProcOut()
    imgproc_out.stacked_rgb = stacked_rgb
...
def inference_resnet_nostage(
    ...
    """
    # Upload the image
    stacked_rgb = torch.from_numpy(imgproc_out.stacked_rgb)
    stacked_rgb = torch.unsqueeze(stacked_rgb, dim=0)
    stacked_rgb = stacked_rgb.cuda()

    # Do forward
    raw_pred = network(stacked_rgb)
    ...
    # Combine them
    keypointxy_pred = np.zeros((2, num_keypoints))
    keypointxy_pred[0, :] = coord_x[0, :, 0]
    keypointxy_pred[1, :] = coord_y[0, :, 0]
    # keypointxy_pred[2, :] = depth_pred[0, :, 0]
    return keypointxy_pred
...
def get_keypoint_xy_real_unit(
        keypointxy_scaled):  # type: (np.ndarray) -> np.ndarray
    """
    ...
    the depth is in mm
    :param keypointxy_scaled:
    :return:
    """
    keypointxy_realunit = np.zeros_like(keypointxy_scaled)
    keypointxy_realunit[0, :] = (keypointxy_scaled[0, :] + 0.5) * parameter.default_patch_size_input
    keypointxy_realunit[1, :] = (keypointxy_scaled[1, :] + 0.5) * parameter.default_patch_size_input
    # keypointxy_realunit[2, :] = (keypointxy_scaled[2, :] * parameter.depth_image_scale) + parameter.depth_image_mean
    return keypointxy_realunit
...
def get_3d_prediction(
    ...
    """
    :param keypoint_xy_patch: (3, n_keypoint) np array.
    ...
    # camera_vertex[2, :] = keypoint_xy_img[2, :].astype(np.float) / 1000.0
    # camera_vertex[0, :] = (keypoint_xy_img[0, :] - parameter.principal_x) * camera_vertex[2, :] / parameter.focal_x
    # camera_vertex[1, :] = (keypoint_xy_img[1, :] - parameter.principal_y) * camera_vertex[2, :] / parameter.focal_y
    # return keypoint_xy_img, camera_vertex
    return keypoint_xy_img
